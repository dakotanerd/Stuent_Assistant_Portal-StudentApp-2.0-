    #base image: 
        ...
        ENV HOME=/app
        ENV DATABASE_URL="postgresql+psycopg2://postgres:postgres@postgres.cv8ucaagqpgx.us-east-2.rds.amazonaws.com/postgres"
        WORKDIR /app
        ...
        RUN openssl req -x509 -newkey rsa:4096 -keyout /etc/ssl/private/key.pem -out /etc/ssl/certs/cert.pem -days 365 -nodes -subj "/C=US/ST=MA/L=Worcester/O=WPI/CN=ec2-3-148-102-153.us-east-2.compute.amazonaws.com"
        COPY . /app
        #set flask to run the production environment
        ENV FLASK_ENV=production
        ENV PATH=/app/.local/bin:$PATH
        ENV CLIENT_ID=f4c6ea46-0826-4c6e-a18a-e174c3262692
        ENV CLIENT_SECRET=sg.8Q~_~l6gp6cUEnwcLZxQb~v3~LeHhswJzzbXq
